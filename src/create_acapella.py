...
def create_acapella(orig_path, instr_path, fn):
    ...
    o_windows_l = np.clip(o_windows_l - (1.2 * i_windows_l), 0, np.amax(o_windows_l))
    o_windows_r = np.clip(o_windows_r - (1.2 * i_windows_r), 0, np.amax(o_windows_r))
    ...
    scw.write(fn + ".wav", sampling_rate, filtered.astype(np.int16))
...
if __name__ == "__main__":
    ...
    create_acapella(orig_path, instr_path, fn)
